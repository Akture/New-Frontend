export default function ExploreVideoCard({ match, onAdd, addedToCart, cartIndex }) {
  return (
    <div
    ...
    >
      <button
        ...
        aria-label={
          addedToCart ? `Remove ${match.title} from your reel` : `Add ${match.title} to your reel`
        }
      ...
      >
        <span className={`absolute inset-0 ${match.gradientDir} ${match.gradient}`}></span>

        <span
          className={`absolute inset-0 transition-colors duration-300 flex items-center justify-center ${
            addedToCart ? 'bg-[#0F172A]/30' : 'bg-[#0F172A]/10 group-hover:bg-[#0F172A]/40'
          }`}
        >
          <span
        ...
        <span className="absolute top-3 left-3 flex gap-2">
          {addedToCart && cartIndex != null && cartIndex >= 0 ? (
            <span className="w-7 h-7 rounded-full bg-gradient-ember text-white text-xs font-black flex items-center justify-center shadow-ember-sm">
              {cartIndex + 1}
            </span>
          ) : (
            <>
